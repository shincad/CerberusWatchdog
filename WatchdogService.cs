...
namespace EnhancedWatchdog
{
    public class WatchdogService : IDisposable
    {
        ...
        public WatchdogService(bool autoStart = false)
        {
            ...
            // 설정 파일 경로를 실행 파일과 같은 위치로 고정 (Single-file app 지원)
            var exeDirectory = AppContext.BaseDirectory;
            settingsFilePath = Path.Combine(exeDirectory, "watchdog_settings.json");
        ...
        public WatchdogSettings LoadSettings()
        {
            try
            {
                if (File.Exists(settingsFilePath))
                {
                    ...
                    // 누락된 필드들을 기본값으로 초기화
                    if (settings.AutoStart == false && !json.Contains("AutoStart"))
                    {
                        settings.AutoStart = true; // 기본값 설정
                    }
                    ...
                    if (loadedData.Processes != null)
                    {
                        foreach (var process in loadedData.Processes)
                        {
                            ...
                            // *** 런타임 정보 초기화 (Watchdog 재시작 시 리셋) ***
                            process.RestartCount = 0;      // 재시작 횟수 리셋
                            process.LastRestart = null;    // 마지막 재시작 시간 리셋
                            process.ProcessId = null;      // 프로세스 ID 리셋
                            process.IsRunning = false;     // 실행 상태 리셋

                            // 누락된 개별 설정 필드들 초기화
                            if (process.IndividualCheckInterval == null && !json.Contains("IndividualCheckInterval"))
                            {
                                process.IndividualCheckInterval = null;
                            }
                            if (process.IndividualBootDelay == null && !json.Contains("IndividualBootDelay"))
                            {
                                process.IndividualBootDelay = null;
                            }
                    ...
                    OnLogMessage($"Settings file has been loaded from: {settingsFilePath}");
                }
                else
                {
                    settings = new WatchdogSettings();
                    OnLogMessage($"Using default settings. Settings file will be created at: {settingsFilePath}");
                }
            }
            catch (Exception ex)
            {
        ...
        public void SaveSettings()
        {
            ...
            for (int attempt = 1; attempt <= maxRetries; attempt++)
            {
                try
                {
                    ...
                    // 방법 1: 직접 쓰기
                    if (TryDirectWrite(json))
                    {
                        LogMessage?.Invoke(this, new LogMessageEventArgs("Settings have been saved."));
                        return;
                    }

                    // 방법 2: 임시 파일 방법
                    if (TryAtomicWriteSimple(json))
                    {
                        LogMessage?.Invoke(this, new LogMessageEventArgs("Settings have been saved."));
                        return;
                    }
                    ...
                    // 방법 3: 강제 쓰기
                    if (TryForceWriteSimple(json))
                    {
                        LogMessage?.Invoke(this, new LogMessageEventArgs("Settings have been saved."));
                        return;
                    }

                    throw new Exception("All save methods failed");
                }
                catch (Exception ex)
                {
                    lastException = ex;
                    LogMessage?.Invoke(this, new LogMessageEventArgs($"Save attempt {attempt} failed: {ex.Message}"));
            ...
            try
            {
                ...
                var json = JsonConvert.SerializeObject(data, Formatting.Indented);
                var emergencyPath = settingsFilePath + $".emergency_{DateTime.Now:yyyyMMdd_HHmmss}.json";
                File.WriteAllText(emergencyPath, json);
                LogMessage?.Invoke(this, new LogMessageEventArgs($"Emergency save: {emergencyPath}"));
                LogMessage?.Invoke(this, new LogMessageEventArgs("Settings have been saved."));
            }
            catch (Exception emergencyEx)
            {
                LogMessage?.Invoke(this, new LogMessageEventArgs($"All save methods failed: {emergencyEx.Message}"));
                throw;
            }
        }
        private bool TryDirectWrite(string json)
        {
            try
            {
                // 파일 속성 리셋
                if (File.Exists(settingsFilePath))
                {
                    var attributes = File.GetAttributes(settingsFilePath);
                ...
                // 검증
                if (File.Exists(settingsFilePath) && File.ReadAllText(settingsFilePath).Contains("Settings"))
                {
                    return true;
                }
                return false;
        ...
        private bool TryAtomicWriteSimple(string json)
        {
            string tempPath = null;
            try
            {
                tempPath = settingsFilePath + ".tmp";
                ...
                return File.Exists(settingsFilePath) && File.ReadAllText(settingsFilePath).Contains("Settings");
            }
            catch
            {
                return false;
        ...
        private bool TryForceWriteSimple(string json)
        {
            try
            {
